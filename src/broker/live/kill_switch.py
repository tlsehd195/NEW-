...
def evaluate_kill_switch_triggers(context: KillSwitchTriggerContext) -> Optional[str]:
    ...
    if (
        context.config.max_daily_loss is not None
        and context.daily_loss is not None
        and context.daily_loss >= context.config.max_daily_loss
    ):
        return "daily_loss_limit_breached"
    if (
        context.config.max_order_frequency_per_hour is not None
        and context.orders_in_last_hour is not None
        and context.orders_in_last_hour > context.config.max_order_frequency_per_hour
    ):
