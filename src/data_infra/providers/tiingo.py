...
from data_infra.provider import PermanentProviderError, bar_available_time, clamp_ingestion_time, corporate_action_available_time
            ...
            if split_factor is not None and float(split_factor) != 1.0:
                ...
                actions.append(
                    CorporateAction(
                        ...
                        # ADR-0216: the earlier of ingestion_time and the
                        # event date's close -- see
                        # data_infra.provider.corporate_action_available_time.
            ...
            if div_cash is not None and float(div_cash) > 0.0:
                actions.append(
                    CorporateAction(
                        ...
                        # ADR-0216 (supersedes the Session 36 ingestion-
                        # time-only rule): same as the SPLIT branch above.
                        available_time=corporate_action_available_time(event_date, ingestion_time),
                        ingestion_time=ingestion_time,
                        provenance=Provenance(
                            source="tiingo", source_dataset=f"tiingo_eod_{security_id}",
