# ADR-0144: Retroactive data quality rescan
...
Immediately after ADR-0143 (the CRITICAL-severity data quality gate),
...
1. ADR-0143's changes exist only on this session's PR branch
   (`claude/phase-11-model-evolution-7hpibr`, PR #19) — the scheduled
   GitHub Actions workflows run off `main`, so until that PR merges,
   every real scheduled run still executes the pre-ADR-0143 code
   entirely.
2. Even after merging, `scripts/ingest_real_market_data.py` only ever
   ...
   — before ADR-0143 existed, or from a run whose findings were
   computed but (pre-ADR-0143) never persisted — is never re-examined
   ...
   ADR-0143's own disclosed Azure Blob Storage access limitation), if
...
`record_quality_issues()` ADR-0143 introduced. Any CRITICAL finding is
...
now to catch up everything ingested before ADR-0143, and again by hand
...
...) ADR-0143's exclusion actually acts on. `known_security_ids` is also
