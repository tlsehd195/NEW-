...
| Live Trading Orchestration | Regime->Prediction->Decision->Sizing->Risk->Order-Validation->`LiveTradingSession.submit` chain, the Live-side equivalent of the Paper Trading row above | **NEW (Session 36, ADR-0070)**: `orchestration.live_runner.run_cycle` mirrors `paper_runner.run_cycle`'s chain exactly, with two forced differences: (1) portfolio state is read from the broker (`adapter.get_account`/`get_positions`), never a local ledger, and raises rather than fabricating $0 cash when the broker reports the account unavailable; (2) `SafetyGateContext` is a REQUIRED caller-supplied base parameter this module never builds itself -- writing it surfaced that NOTHING in `src/` currently computes real values for most of that context's fields (`risk_health`/`account_state_known`/`position_state_known`/`model_state_valid`/`configuration_integrity_valid`/`max_turnover`/`approval`); every existing caller is a test fixture hardcoding a "passing" context. `run_cycle` only derives `order_validation_status` itself (from each security's own real validation result) -- everything else is the caller's responsibility, unchanged. Tested exclusively against `broker.mock.MockBrokerAdapter`, never `broker.toss.adapter.TossBrokerAdapter` -- no code anywhere may call this module against a real broker with a real `SafetyGateContext` until the gap above is separately closed. **UPDATED (Session 36 continued, ADR-0071)**: on review, several of the "caller-supplied" fields turned out to be things `run_cycle` can know with total certainty rather than a genuinely open question -- `config`/`broker_capabilities`/`kill_switch_engaged`/`account_state_known`/`position_state_known` are now ALSO derived by `run_cycle` itself (from `session.config`/`session.adapter.get_capabilities()`/`session.is_kill_switch_engaged()`/the fact that `_live_portfolio_view` did not raise), closing a real risk where a caller's stale or wrong placeholder for these could have silently passed the gate (e.g. claiming `kill_switch_engaged=False` when it was really engaged). Only `risk_health`/`model_state_valid`/`configuration_integrity_valid`/`max_turnover`/`approval`/`required_capabilities` remain genuinely caller-supplied now, each for a distinct, real reason (see ADR-0071). **UPDATED (Session 36 continued, ADR-0072)**: `docs/specifications/PHASE-16-live-trading.md` section 5 turned out to already precisely define `model_state_valid`/`configuration_integrity_valid` -- `orchestration.live_safety_gate_inputs.compute_model_state_valid`/`compute_configuration_integrity_valid` now implement both exactly per spec, as pure, tested functions (reading an already-recorded `ModelStatusTransition`, never constructing one -- never imports `evolution.criteria`, mirroring `broker.live.*`'s own self-imposed restriction). `run_cycle` still cannot call either itself: `model_state_valid` needs a real `candidate_id`, and nothing yet ties `run_cycle`'s deterministic components to any specific `CandidateModelArtifact` (a separate, still-open architecture question); `configuration_integrity_valid` needs an operator-supplied pinned hash. `risk_health` remains fully open (no real failure-rate tracking exists for `risk_engine.assess()`, which structurally returns REJECT rather than raising) | `src/orchestration/live_runner.py`; `src/orchestration/live_safety_gate_inputs.py`; `tests/orchestration/test_live_runner.py`; `tests/orchestration/test_live_orchestration_boundary.py`; `tests/orchestration/test_live_safety_gate_inputs.py`; `tests/orchestration/test_live_safety_gate_inputs_boundary.py`; `docs/decisions/ADR-0070`; `docs/decisions/ADR-0071`; `docs/decisions/ADR-0072` | **STRUCTURE IMPLEMENTED & TESTED (MockBrokerAdapter only); 7 of 13 SafetyGateContext fields derived from real state at call time, 2 more have real tested pure functions available but not yet wired (need a candidate_id / a pinned hash only an operator can supply). 1 field (`risk_health`) has no real computation path designed yet** | **Yes** (for actual Live use -- the orchestration chain itself is not the blocker, the unbuilt/unwired SafetyGateContext inputs are) | **Yes** -- which candidate_id (if any) governs a given live run, so `model_state_valid` can actually be computed automatically? Where does the pinned `configuration_version` for `configuration_integrity_valid` get set and by whom? What does a meaningful `risk_health` failure signal even mean for a risk engine that structurally returns REJECT rather than raising? | Design "which candidate governs this live run" (needed for `model_state_valid` to ever be wired automatically); decide an operational process for pinning/rotating `configuration_integrity_valid`'s reference hash; design a real `risk_health` failure signal for the Risk Engine |
