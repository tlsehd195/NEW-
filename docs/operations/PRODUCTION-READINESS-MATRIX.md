...
| Learning | Experience -> Training Dataset provenance-safe; PAPER cannot become LIVE | Re-verified this phase, including a bypass-the-filter defense-in-depth test. **UPDATED (Session 37, ADR-0086)**: this row's own provenance-safety guarantee was always real, but until this session nothing fed it real Paper Trading data to be safe ABOUT -- `orchestration.paper_runner.run_cycle` never recorded a `DecisionSnapshot`/`TradeRecord` for anything it did, so `build_experience_records` had zero real Paper experience to ever read despite the daily GitHub Actions Paper Trading scheduler having been live for several sessions. `src/trade_journal/paper_adapter.py` (new) closes this -- `run_cycle`'s opt-in `trade_journal`/`journal_state` parameters record every checkpoint's real Decision and any real Trade, `scripts/run_paper_trading_cycle.py` now does this by default, and new `scripts/run_learning_cycle.py` is the missing read side: builds real `ExperienceRecord`s from a real `--paper-store` and runs `learning.pipeline.run_learning_pipeline`, persisting every stage through real DuckDB-backed `storage.learning_repository` classes. Defaults to `MeanRewardBaselineTrainer`, not `LinearRegressionTrainer` -- no Strategy/DecisionAgent in this codebase sets `OrderIntent.features` yet (ADR-0048's own documented gap), so a real linear-regression run against real Paper data today honestly reports `fitted=False`, verified by a real test rather than asserted in prose. Live Trading is NOT wired the same way (Live activation remains structurally blocked, see the Live Trading Orchestration/Walk Forward rows) -- the same adapter shape is the natural next step once Live actually runs, not built speculatively now | `tests/integration/test_paper_learning_readiness_lineage.py::TestPaperToLearningProvenanceSafety`; `tests/trade_journal/test_journal_paper_adapter.py`; `tests/orchestration/test_paper_runner.py::TestTradeJournalWiring`; `tests/orchestration/test_run_paper_trading_cycle_cli.py::TestTradeJournalWiring`; `tests/orchestration/test_run_learning_cycle_cli.py`; `docs/decisions/ADR-0086` | **PASS** (provenance safety); **NOW ACTUALLY FED REAL PAPER DATA** (previously structurally correct but empty) | No | No | Populate real `OrderIntent.features` from some Strategy/DecisionAgent when one is worth building, so `LinearRegressionTrainer` (or a future feature-based trainer) has something real to fit; wire the same adapter shape for Live once Live activation is no longer blocked |
