...
| Risk (policy completeness) | Every named risk limit DEFINED or explicitly deferred with a human decision on file | 1 DEFINED, 6 INHERITED (never re-approved for real capital specifically), 3 UNDEFINED, 3 BLOCKING (no field exists). Reasoned *proposed* values exist for the 3 UNDEFINED fields (`max_daily_loss` = 2% of eventual initial capital, `max_turnover` = 2.0, `max_order_frequency_per_hour` = 6, revised down from Phase 20's original 3.0/30 -- see `LIVE-RISK-POLICY.md`'s "Revised" table) -- proposals only, still not ratified (`max_daily_loss` genuinely cannot be a concrete number until initial Live capital itself is decided). **UPDATED (Session 36, ADR-0045)**: the design-level "should `None` block Live" question is now resolved as Option B for all three fields (`max_daily_loss`/`max_order_frequency_per_hour` since Phase 22, `max_turnover` newly this session) -- `evaluate_safety_gate` now structurally blocks Live whenever any of the three is unset, closing the asymmetry this row used to describe. **UPDATED further (Session 36 continued)**: #1/#6/#7/#11 (`max_daily_loss=5%`, `max_turnover=2.0`, `max_order_frequency_per_hour=6`, `max_consecutive_failures=5`) are now RATIFIED (ADR-0060/ADR-0065) -- real numbers, not proposals, though still not code-applied to `DEFAULT_LIVE_TRADING_CONFIG` by design (a human passes them explicitly whenever a real Live config is constructed). #14 (withdrawal policy) is RESOLVED: no withdrawal, ever -- every realized gain is reinvested (ADR-0076), which is not a number to ratify, it removes the item entirely. #5 (`max_sector_weight=25%`) and #10 (`max_order_notional=$1,000`) now have reasoned PROPOSED values on file (ADR-0076), awaiting the same ratify-or-revise step #1/#6/#7 already went through. #15 (Live cash buffer) is now also RESOLVED, interim (ADR-0079): keep the inherited 5% floor for now, explicitly revisit with a regime-conditional buffer once real Live operating data exists -- not built yet, on RULE 0.8 grounds (designing that mapping before any real data exists to inform it would be premature policy-tuning). **UPDATED further (Session 36 continued, ADR-0080)**: #5 (`max_sector_weight=25%`) and #10 (`max_order_notional=$1,000`) are now also RATIFIED, real numbers -- same "not yet code-applied to the shared default" treatment as #1/#6/#7/#11. Every numbered item in `LIVE-RISK-POLICY.md` now has either a RATIFIED value, a DEFINED/INHERITED value, or a RESOLVED policy (#14/#15) -- none remain fully open except item B (Toss operational verification / KRW-USD FX), explicitly deferred by the account owner | `docs/operations/LIVE-RISK-POLICY.md`; `src/broker/live/safety_gate.py`; `docs/decisions/ADR-0045`; `docs/decisions/ADR-0076`; `docs/decisions/ADR-0079`; `docs/decisions/ADR-0080` | **PARTIAL** | No (all numeric policy decisions are now made; only Toss operational verification remains, tracked separately in the rows above) | No -- remaining rows above (Toss capability verification) already carry their own separate Human Decision Required flags | None for risk-policy numbers; Toss operational verification (rows above) remains the only decision-pending item |
| Kill Switch | Auto-engage on critical conditions; AI cannot release | Implemented; `data_health` trigger added this phase. **Correction (Batch H, independent audit item 1)**: at the time this row was written, `evaluate_kill_switch_triggers` had zero production callers anywhere in `src/`/`scripts/` -- "Auto-engage" was true only inside its own test file, never in any real execution path. Closed by Batch E (ADR-0182): `orchestration.live_runner.run_cycle` now calls `engage_kill_switch` automatically when a caller supplies a `KillSwitchTriggerContext` and a trigger fires. Still has zero observable production effect, since no real Live driver exists to be that caller. | `tests/broker/live/test_live_kill_switch.py`; `tests/orchestration/test_live_runner.py::TestKillSwitchAutoEngage`; `docs/decisions/ADR-0182` | **PASS for the mechanism itself; NOT YET WIRED to any real production caller** | No | No | Build a real Live driver that constructs a `KillSwitchTriggerContext` from real broker/risk/monitoring health before Live ever actually runs |
| Monitoring | Portfolio/broker/data/model health collected, persisted, alertable | Phase 15 equity/PnL/drawdown gap closed in Phase 17; Paper performance-report metrics (Sharpe/Sortino/etc.) built in Phase 18 but deliberately not fed into Monitoring (a periodic report, not a component-health signal -- ADR-0024 decision, `docs/specifications/PHASE-18-paper-performance-and-validation.md` section 6). **Correction (Batch H, independent audit item 10)**: "collected, persisted, alertable" + **PASS** was true only in the narrow sense that the collector functions and repository classes existed and were exercised by tests -- a repo-wide grep found ZERO production callers of any collector or of any `storage.monitoring_repository` class outside their own test files, so nothing this project's real daily cron ever produced was actually persisted anywhere durable. Closed for `prediction`/`decision`/`sizing`/`risk`/`account` by Batch F (`scripts/run_monitoring_sweep.py`, wired into `paper_trading_cycle.yml`'s daily run over the trailing 7 days). Still NOT wired for `broker` (no real `BrokerRequestRecord`/`BrokerResponseRecord` data exists in `--paper-store` -- `PaperTradingSession.submit()` bypasses `broker.pipeline` entirely), `data_quality` (needs the separate market-data catalog, `--db-path`, not `--paper-store`), `ai_gateway`/`learning`/`model_evolution` (no real production calls / separate weekly cron), or drift detection (no baseline-vs-current comparison window built yet). | `tests/integration/test_paper_monitoring_integration.py`; `tests/scripts/test_run_monitoring_sweep.py`; `tests/storage/test_monitoring_repository.py`; `docs/decisions/ADR-0183` | **PASS for prediction/decision/sizing/risk/account (Batch F, real production wiring); NOT YET WIRED for broker/data_quality/ai_gateway/learning/model_evolution/drift** | No | No | Wire `data_quality` (accept `--db-path`), `broker` (once `PaperTradingSession` routes through `broker.pipeline`), and drift detection (build a real baseline window) |
...
| Paper Trading | Order->Fill->Journal->Experience traced with real code; performance evaluated beyond raw return | Lineage traced in Phase 17 (5 scenarios A-E); Phase 18 built `broker.paper.performance` (Sharpe/Sortino/Calmar/drawdown/turnover/cost/slippage/win-rate/benchmark) and re-verified the lineage under 8 further scenarios (A-H) including partial fill, insufficient cash/position, duplicate order id, broker failure, and drawdown. **UPDATED (Session 36, ADR-0067)**: `orchestration.paper_runner.run_cycle` is the first real Regime->Prediction->Decision->Sizing->Risk->Order-Validation->`PaperTradingSession.submit` pipeline in this codebase -- previously `PortfolioRiskEngine.assess` had zero callers anywhere in `src/` (only exercised by tests). Still not "a real, running Trading Engine loop" (no scheduler/timer); `run_cycle` is called once per checkpoint by whatever drives it, matching `BacktestEngine.run()`'s own per-checkpoint loop shape. **UPDATED further (Session 36 continued, ADR-0068)**: the `value_history` limitation above is closed -- `orchestration.paper_runner.PaperRunnerState` carries a real running portfolio-value series across successive `run_cycle` calls (opt-in; `state=None` preserves the original disabled behavior). The five upstream stages (Prediction/Regime/Decision/Sizing/Risk) can now be persisted through optional repository parameters reusing the exact classes `tests/integration/test_risk_lineage.py` already established. `scripts/run_paper_trading_cycle.py` is a real, tested (network-free, run end to end against a seeded DuckDB catalog) CLI that loops `run_cycle` over a real trading-day range against a real market-data catalog, sourcing `sector_by_security` from the chosen universe's own real `SymbolMetadata.sector` data -- still not an always-on scheduled process (runs once over a fixed window, then exits; a real external scheduler invoking it repeatedly remains out of scope). **UPDATED (Session 36 continued, ADR-0073)**: repeated invocation of this script against the same `--paper-store` is now actually SAFE, which nothing had verified before -- `--resume` skips already-processed checkpoints (detected from real persisted state) and reconstructs `value_history` from real prior portfolio values; `PaperTradingSession.restore()` (existing, previously never called by this script) now rebuilds the ledger from persisted state unconditionally. Building `--resume`'s own tests surfaced a real, pre-existing, non-`--resume`-specific bug: `DriftPredictor`/`RegimeDetector`/`BaselineRuleDecisionAgent`/`DeterministicPositionSizer`/`DeterministicPortfolioRiskEngine` each reset their own ID counter to 1 every fresh process, and every affected repository enforces a real PRIMARY KEY on that ID -- meaning this script was never actually safe to invoke a second time against a non-empty store at all, resume or not. Fixed at the source (all 5 classes now accept an optional `starting_id`, default 1, unchanged behavior for every existing caller) and wired so the CLI always seeds every counter from the store's own real persisted max. **UPDATED (Session 36 continued, ADR-0081)**: the "where does this run" gap was first designed around an Oracle Cloud Always Free VM (`docs/operations/ORACLE-CLOUD-DEPLOYMENT.md` + `scripts/deploy/oracle_vm_bootstrap.sh`, still present in the repo as a documented alternative, unused). **SUPERSEDED (Session 36 continued, ADR-0082)**: the account owner found the manual VM setup too much operational friction and asked to use GitHub Actions instead -- this time with the ephemeral-runner state-persistence gap actually solved rather than left open: `.github/workflows/paper_trading_cycle.yml` round-trips both `--db-path` and `--paper-store` as GitHub Actions artifacts across runs (restored via `gh api` against the repo's own `/actions/artifacts` endpoint at the start of each run, re-uploaded with `if: always()` at the end), never committing DuckDB files into git (preserving the existing `.gitignore` policy). Re-ingestion each run reuses the same fixed `--start`/`--end=today` range against the restored catalog, safe because `DuckDBDataRepository.append_bars` already dedupes by natural key (verified in source, not assumed). The only manual step left is adding `MARKET_DATA_API_KEY` as a GitHub Actions repository secret (`docs/operations/GITHUB-ACTIONS-DEPLOYMENT.md`) -- this session cannot do that on the account owner's behalf. **UPDATED (Session 36 continued, PR #1 + ADR-0083)**: the account owner tried to run it and found it missing from the Actions UI entirely -- root cause was that GitHub Actions only lists/triggers a workflow once its file exists on the repository's default branch (`main`), which this work had not yet reached; resolved by opening and merging PR #1 (33 commits, `claude/phase-11-model-evolution-7hpibr` -> `main`). Separately, the account owner asked what GitHub Actions' actual downsides are and whether they'd been solved -- audited honestly: they had only been disclosed, not solved. Verified against GitHub's own documented behavior (not assumed): a repository's scheduled workflows auto-disable after 60 days with no repository **commit** activity, and a workflow's own successful runs do not count as that activity. `.github/workflows/keepalive.yml` now makes a trivial monthly commit (a heartbeat file only, never `data/` or a DuckDB path) specifically to prevent this, which transitively also protects the 90-day artifact-expiry risk (artifacts only expire if the scheduler stops getting the chance to run at all). **UPDATED (Session 36 continued, ADR-0085)**: the account owner's first real manual `workflow_dispatch` run (699/699 checkpoints, 1468 orders submitted and filled) also reported `ingestion_status: PARTIAL_SUCCESS` -- 10 of 88 symbols, including `SPY` the benchmark, got zero bars. Root cause, confirmed rather than assumed (the 10 failures cluster at the end of the universe's symbol order): re-requesting the FULL `$START_DATE`..today range (~2.5 years) every single run, as ADR-0082 designed it, is large enough to exhaust Tiingo's rate limit partway through -- and recurs every run, not just the first. Fixed with `scripts/compute_incremental_ingestion_start.py`: queries the restored catalog's own last known bar and requests only a 7-day-overlap window forward from it, falling back to the fixed `$START_DATE` only when the catalog is empty. Reduces daily request volume from ~2.5 years to ~1 week per symbol once the catalog is populated | `tests/broker/paper/test_paper_performance.py`; `tests/integration/test_paper_performance_scenarios.py`; `tests/orchestration/test_paper_runner.py`; `tests/orchestration/test_orchestration_boundary.py`; `tests/orchestration/test_run_paper_trading_cycle_cli.py`; `tests/deploy/test_oracle_vm_bootstrap_script.py`; `tests/deploy/test_paper_trading_cycle_workflow.py`; `tests/deploy/test_keepalive_workflow.py`; `tests/deploy/test_compute_incremental_ingestion_start.py`; `docs/decisions/ADR-0067`; `docs/decisions/ADR-0068`; `docs/decisions/ADR-0069`; `docs/decisions/ADR-0073`; `docs/decisions/ADR-0081`; `docs/decisions/ADR-0082`; `docs/decisions/ADR-0083`; `docs/decisions/ADR-0085` | **PASS** | No | No | Adding the `MARKET_DATA_API_KEY` GitHub Actions secret is a manual step only the account owner can perform (already done); the next scheduled/manual run should be watched to confirm the incremental-window fix actually keeps every symbol (including SPY) populated, since this session cannot execute the workflow itself. A Live-side equivalent now exists -- see the Live Trading Orchestration row below |
| Live Trading Orchestration | Regime->Prediction->Decision->Sizing->Risk->Order-Validation->`LiveTradingSession.submit` chain, the Live-side equivalent of the Paper Trading row above | **NEW (Session 36, ADR-0070)**: `orchestration.live_runner.run_cycle` mirrors `paper_runner.run_cycle`'s chain exactly, with two forced differences: (1) portfolio state is read from the broker (`adapter.get_account`/`get_positions`), never a local ledger, and raises rather than fabricating $0 cash when the broker reports the account unavailable; (2) `SafetyGateContext` is a REQUIRED caller-supplied base parameter this module never builds itself -- writing it surfaced that NOTHING in `src/` currently computes real values for most of that context's fields (`risk_health`/`account_state_known`/`position_state_known`/`model_state_valid`/`configuration_integrity_valid`/`max_turnover`/`approval`); every existing caller is a test fixture hardcoding a "passing" context. `run_cycle` only derives `order_validation_status` itself (from each security's own real validation result) -- everything else is the caller's responsibility, unchanged. Tested exclusively against `broker.mock.MockBrokerAdapter`, never `broker.toss.adapter.TossBrokerAdapter` -- no code anywhere may call this module against a real broker with a real `SafetyGateContext` until the gap above is separately closed. **UPDATED (Session 36 continued, ADR-0071)**: on review, several of the "caller-supplied" fields turned out to be things `run_cycle` can know with total certainty rather than a genuinely open question -- `config`/`broker_capabilities`/`kill_switch_engaged`/`account_state_known`/`position_state_known` are now ALSO derived by `run_cycle` itself (from `session.config`/`session.adapter.get_capabilities()`/`session.is_kill_switch_engaged()`/the fact that `_live_portfolio_view` did not raise), closing a real risk where a caller's stale or wrong placeholder for these could have silently passed the gate (e.g. claiming `kill_switch_engaged=False` when it was really engaged). **UPDATED (Session 36 continued, ADR-0074)**: all three of `risk_health`/`model_state_valid`/`configuration_integrity_valid` are now overridden by `run_cycle` itself, OPT-IN, whenever the caller supplies the real inputs each needs -- `risk_health` from `LiveRunnerState`'s own real running REJECT-rate counters (fed into the existing `monitoring.health.evaluate_health_from_failure_rate`, `MonitoringComponent.RISK` -- `MonitoringConfig`'s own comment already named "Risk rejections" as this component's intended signal, not an exception rate) when `state` is supplied; `model_state_valid` when BOTH `candidate_id` and `model_status_repository` are supplied; `configuration_integrity_valid` when `pinned_configuration_version` is supplied. Omitting the right inputs for any of the three leaves the caller's own value on the base `gate_context` untouched, exactly as before -- nothing is fabricated. Only `max_turnover`/`approval`/`required_capabilities` remain UNCONDITIONALLY caller-supplied now, each for the distinct structural reason ADR-0071 already documented. **UPDATED further (Session 36 continued, ADR-0077/ADR-0078)**: the two open questions this row used to list are now both closed. "Which candidate_id governs a live run" was analyzed, not designed around -- this project's own research found 0 of 30 strategy candidates reaching `VALIDATED` evidence level, so `model_state_valid` staying caller-supplied (with no real `candidate_id` to pass) is the system correctly reflecting that no strategy has earned Live approval, not a gap; the real open question underneath is a research one (does any strategy ever reach `VALIDATED`), which this project's existing validation infrastructure already answers honestly on its own. "Where does an operator set the configuration pin" is answered: `scripts/print_live_configuration_version.py` computes it with the exact code `run_cycle`'s own check compares against, and `LIVE-TRADING-RUNBOOK.md`'s "Configuration Validation" section names the process (compute, record in the activation log, rotate by re-running and adding a new entry) | `src/orchestration/live_runner.py`; `src/orchestration/live_safety_gate_inputs.py`; `tests/orchestration/test_live_runner.py`; `tests/orchestration/test_live_orchestration_boundary.py`; `tests/orchestration/test_live_safety_gate_inputs.py`; `tests/orchestration/test_live_safety_gate_inputs_boundary.py`; `docs/decisions/ADR-0070` through `ADR-0080` | **STRUCTURE IMPLEMENTED & TESTED (MockBrokerAdapter only); ALL 10 of 13 computable SafetyGateContext fields now have a real derivation path (7 unconditional, 3 opt-in given the right caller inputs). 3 fields (`max_turnover`/`approval`/`required_capabilities`) remain unconditionally caller-supplied by structural necessity, not by gap** | **Yes** (for actual Live use -- the orchestration chain itself is not the blocker; a real caller must still supply `approval`/`required_capabilities`/`max_turnover`, none of which this project can or should supply on its own) | No -- both prior open design questions are resolved (ADR-0077/ADR-0078); registering the ADR-0075 crontab entry on a real machine is an operational step, not a decision | Register the ADR-0075 crontab entry (or equivalent) on a real machine when ready to run the Paper-side scheduler; supply real `approval`/`required_capabilities`/`max_turnover` whenever a real Live invocation is actually attempted |
| Learning | Experience -> Training Dataset provenance-safe; PAPER cannot become LIVE | Re-verified this phase, including a bypass-the-filter defense-in-depth test. **UPDATED (Session 37, ADR-0113)**: this row's own provenance-safety guarantee was always real, but until this session nothing fed it real Paper Trading data to be safe ABOUT, and a second, deeper gap meant even feeding it data would not have worked. `orchestration.paper_runner.run_cycle` (ADR-0096/ADR-0097) wrote real `TradeRecord`s but never a `DecisionSnapshot`, and never passed `realized_pnl`/`realized_return`/`holding_period` -- so `learning.cleaning.DataCleaner.clean` excluded EVERY real Paper Trading trade, first with `"missing_decision"` (the `decision_id` join never resolved) and, even had that resolved, `"no_realized_outcome"` next (a real closing SELL's PnL was always `None`). Fixing only one of the two would not have made the Learning Engine usable at all. `run_cycle` now records a real, joinable `DecisionSnapshot` (via `record_decision`, `order=` deliberately left `None` -- `ValidatedOrder`/`backtest.orders.Order` are structurally incompatible types, confirmed by a real crash caught before this ever reached a caller) whenever a validated order results, and every `TradeRecord` for that security this cycle uses its real `snapshot_id` -- closing ADR-0097's own documented "not joinable" limitation. A closing SELL's `realized_pnl`/`realized_return` are computed from `portfolio.positions[...].average_cost` (real cost basis as of the start of the cycle) and `holding_period` from a real replay of the security's own Trade Journal history (`_position_opened_at`) -- both `None`, never fabricated, when the inputs to compute them are themselves unavailable. New `scripts/run_learning_cycle.py` is the missing read side: builds real `ExperienceRecord`s from a real `--paper-store` and runs `learning.pipeline.run_learning_pipeline`, persisting every stage through real DuckDB-backed `storage.learning_repository` classes. **Proven end to end, not just plumbed**: a real Paper Trading run against a reversal price fixture (rise then decline, forcing a real closing SELL) now produces a real `dataset_quality_status="OK"` dataset and a `COMPLETED` training run with a real, non-trivial `test_metrics.mean_label` -- the first time this project's Learning Engine has ever actually trained on real Paper Trading experience. Defaults to `MeanRewardBaselineTrainer`, not `LinearRegressionTrainer` -- at the time this row was written, no Strategy/DecisionAgent in this codebase set `OrderIntent.features` yet (ADR-0048's own documented gap), so a real linear-regression run against real Paper data still honestly reported `fitted=False`, verified by a real test rather than asserted in prose. **Correction (Batch H, independent audit item 8)**: this specific claim is now stale -- ADR-0141 (Session 38) wired a real `_decision_features(prediction, regime)` helper into `paper_runner.record_decision(...)` (a different, more direct mechanism than the `OrderIntent.features`/`Strategy` gap this row originally cited, and one `orchestration.paper_runner`'s real production decision path -- `decision.agent.DecisionAgent`, not an `OrderIntent`-producing `Strategy` -- actually goes through), so `--trainer linear_regression` can now produce a real, non-trivial fit against real Paper Trading data; see `docs/decisions/ADR-0141` and `tests/orchestration/test_run_learning_cycle_cli.py`'s own updated test docstrings. An earlier, independent attempt at this same problem (`src/trade_journal/paper_adapter.py`, a separate branch's own ADR-0086) was found to duplicate ADR-0096/ADR-0097's already-merged write-side wiring once both landed on `main` together -- reconciled by deleting the duplicate module and building this session's fix directly into the already-merged `orchestration.paper_runner.run_cycle` instead. Live Trading is NOT wired the same way (Live activation remains structurally blocked, see the Live Trading Orchestration/Walk Forward rows) -- the same fix shape is the natural next step once Live actually runs, not built speculatively now | `tests/integration/test_paper_learning_readiness_lineage.py::TestPaperToLearningProvenanceSafety`; `tests/orchestration/test_paper_runner.py::TestTradeJournalWriteSide`; `tests/orchestration/test_run_learning_cycle_cli.py`; `tests/orchestration/test_run_learning_cycle_cli.py::TestFullLoopWithARealClosedTrade`; `docs/decisions/ADR-0096`; `docs/decisions/ADR-0097`; `docs/decisions/ADR-0113` | **PASS** (provenance safety); **PROVEN END TO END WITH REAL DATA** (previously structurally correct but empty, then briefly structurally wired but still permanently unusable) | No | No | (Done, ADR-0141 -- see Batch H correction above) wire the same fix shape for Live once Live activation is no longer blocked |
...
| Walk Forward | Rolling-window re-validation / PBO / Deflated Sharpe | **UPDATED (Session 36)**: `strategy_research.pbo_dsr.compute_pbo`/`compute_dsr_for_all_candidates` are implemented and have now run against real data multiple times. `run_long_horizon_validation.py` reports `EvidenceLevel` per candidate (CANDIDATE/ROBUSTNESS_PENDING) automatically once `assess_pbo_dsr_applicability`'s trigger conditions are met -- this has now fired for real, repeatedly. **UPDATED (Session 36 continued, ADR-0051/ADR-0052)**: 20 more literature candidates were raw-IC-screened and wired into the same pool (now 28 candidates total, up from 8, real 63-symbol `RESEARCH_UNIVERSE_STAGE3` run, PBO=14.29% across 70 CSCV splits). `size` briefly reached `CANDIDATE` before a concentration-report check found its held-out TEST return was 76.3% one security (an oil-price-supercycle-era energy name) -- traced to a genuine research-methodology gap (`top_n=5`, too narrow a portfolio breadth to average out one name's luck, found and fixed uniformly across all 28 candidates as `top_n=10`, not tuned per-candidate); re-verification correctly dropped `size` below the fold-consistency bar. Two candidates (`altman_z`, `rank_average_ensemble`) reach `evidence=CANDIDATE` at the corrected breadth, but both show strongly negative held-out TEST results (-25.80%/-15.19% net), so neither should be read as "found a validated strategy." **UPDATED (Session 36 continued further, ADR-0057)**: 2 more candidates (`combined_factor`, `idiosyncratic_volatility`) were wired in and the pool re-run against the wider 87-symbol `RESEARCH_UNIVERSE_STAGE4` (30 candidates total, PBO=7.14%). `altman_z`/`rank_average_ensemble` remain the only 2 reaching `CANDIDATE`, with materially unchanged TEST-negative results -- reproducing under an independently-expanded universe strengthens the "hold, do not promote" conclusion. Neither new candidate reached `CANDIDATE`; `combined_factor` in particular showed the worst fold-consistency of all 30 (20% positive folds) alongside a positive TEST result and almost no trading activity (8 trades) -- read as a sample-sparsity artifact from its own 9-leg all-or-nothing coverage requirement, not a genuine signal, per `docs/research/STRATEGY-VALIDATION-REPORT.md`'s Phase 33 Addendum Section H. **UPDATED (Session 36, ADR-0045)**: the "should CANDIDATE be required before Live" question is now resolved -- `broker.live.approval.LiveActivationApproval.strategy_evidence_reviewed` (new required field) structurally requires the human activator to attest the strategy reached CANDIDATE-or-better AND that its TEST result specifically (not just the label) was reviewed, directly motivated by this exact class of divergence. **UPDATED (Session 36 continued, ADR-0084)**: after the account owner asked whether the currently-running Paper Trading scheduler (a naive `DriftPredictor`/`BaselineRuleDecisionAgent` baseline, explicitly documented as "not a claim of skill") could double as strategy research -- answered honestly: no, it cannot; the scheduler executes a single fixed rule forward in real time and does not search any factor space, while research is done by testing NEW candidates against the ALREADY-ingested historical range. Given how exhaustively the classic factor literature was already covered (30 candidates: value/quality/momentum-reversal/low-risk/size/liquidity/financial-health/shareholder-yield), a genuinely new category was identified and wired in BEFORE any real result: `sue_score` (Foster, Olsen & Shevlin 1984 Standardized Unexpected Earnings / Bernard & Thomas 1989 post-earnings-announcement drift), needing quarterly (not fiscal-year) `EarningsPerShareDiluted`, added to `ingest_fundamentals_data.py`'s default concepts at zero additional real network cost. A second new category (insider-trading signals, SEC Form 4) is planned next, pending a data-feasibility check (confirmed accessible in principle via a real, manual EDGAR fetch -- Form 4 XML is well-structured, but distinguishing genuine open-market purchases from Rule 10b5-1 pre-scheduled plan transactions is a real methodology requirement the first real sample surfaced). **UPDATED (Session 36 continued)**: real raw IC (`mean_ic=+0.0189`, sign matches literature, near-zero magnitude, `positive_ic_ratio=48.53%`) and real walk-forward/PBO/DSR results are in for `sue` -- `ROBUSTNESS_PENDING`, not `CANDIDATE` (`positive_fold_ratio=43.33%` vs the 60% bar, `deflated_sharpe_ratio=0.419` vs the 0.95 bar; held-out TEST net=-10.26%, Sharpe=-0.04). The 31st candidate to land here, recorded honestly per RULE 0.8 rather than retuned after seeing the result. Overall pool unchanged by its addition: PBO=7.14%, `altman_z`/`rank_average_ensemble` remain the only 2 reaching `CANDIDATE`, both still TEST-negative and held | `src/strategy_research/pbo_dsr.py`; `src/strategy_research/factor_strategy.py`; `src/strategy_research/factor_scores.py` (`sue_score`); `src/broker/live/approval.py`; `docs/research/walk-forward-pbo-deflated-sharpe.md` section 9; `docs/decisions/ADR-0043-ml-first-model.md` Decision 7; `docs/decisions/ADR-0045`; `docs/decisions/ADR-0051`; `docs/decisions/ADR-0052`; `docs/decisions/ADR-0057`; `docs/decisions/ADR-0084`; `docs/decisions/ADR-0086`; `docs/decisions/ADR-0087`; `docs/decisions/ADR-0088`; `docs/decisions/ADR-0089`; `docs/research/STRATEGY-VALIDATION-REPORT.md` Section G, "Phase 33 Addendum", and "Session 36 continued Addendum" | **RESEARCHED, IMPLEMENTED, RUN AGAINST REAL DATA (31 candidates). Zero candidates VALIDATED (a distinct, higher bar than CANDIDATE that requires explicit human review, not automated)** **UPDATED (2026-09-25, `run_full_validation.yml` real run against `research-catalogs-v1`, `docs/research/reports/full-validation-20260925T160732Z.json`)**: the pool has grown to **51 candidates** (`research_log_summary.candidate_count=51`) -- still **zero `VALIDATED`**, all 51 classified `INCONCLUSIVE` at the walk-forward/PBO stage (PBO=18.57% across 70 CSCV splits, 8 groups). The "31 candidates" figure directly above is a Session-36-era snapshot, left unchanged here per this file's append-only convention -- it is stale and must not be cited as the current count. (Caught during a 2026-09-25/26 external-audit verification round: the audit report itself initially cited "31" by quoting this exact stale sentence without checking the primary-source JSON first -- corrected in its own addendum after independent re-verification.) | No (not independently blocking Live, since it gates model *quality* confidence, not a hard safety boundary) | No -- resolved as a structural attestation requirement (like `checklist_completed`, this is not a computationally-verified gate, but skipping it now requires deliberately lying on the field rather than merely forgetting a step) | Continue running `scripts/run_long_horizon_validation.py` against real data as the universe/feature set evolves (e.g. a real insider-trading candidate, once its own data pipeline is built); a human must still explicitly review any candidate before treating it as VALIDATED -- no code path does this automatically. **UPDATED (Session 36 continued further, ADR-0086)**: the "once its own data pipeline is built" note above is now done -- the second new-category candidate planned alongside `sue` is fully implemented: `InsiderTransaction` model + `DuckDBInsiderRepository` (point-in-time-safe on the real SEC filing date, never the trade date), 5 new SEC Form 4 methods on `SecEdgarFundamentalsProvider` (Tier 1, verified byte-for-byte against the real AAPL sample captured earlier this session), `insider_buying_score` (Lakonishok & Lee 2001 / Seyhun 1986 net insider-purchase ratio over a trailing 6 months, restricted to genuinely discretionary Code P/S transactions with `is_10b5_1_plan == False` -- a restriction decided directly from that same real sample, which turned out to be a Rule 10b5-1 pre-scheduled sale, before any IC result exists), and `scripts/ingest_insider_transactions.py`. Wired blind into both research scripts via a new independent `--insider-db-path` flag (`compute_fundamentals_ic_from_catalog.py`'s `--score insider_buying`; `run_long_horizon_validation.py`'s `_INSIDER_FACTOR_CANDIDATES`, the pool's 32nd candidate) -- same RULE 0.8 discipline as every earlier candidate. This session's own network is still blocked to `sec.gov`, so no real IC/walk-forward result exists yet; the account owner needs to run the new ingestion script in their own environment before one can be computed. **UPDATED (Session 36 continued further, ADR-0088)**: the account owner ran real ingestion (4877 transactions, 87 symbols) then the raw-IC screen -- `observations=0`. Diagnosed against the real catalog: the entire ingested date range (2023-09..2026-09) sat almost completely inside `strategy_research.locked_windows.TEST_1`, since `fetch_form4_filing_list` made only one EDGAR request per symbol (40 most recent filings, nowhere near this project's 2010 raw-IC start for 87 actively-traded large-caps). Fixed with real pagination -- `fetch_form4_filing_list` gained `before_date` (EDGAR's own `dateb` parameter), and a new `_fetch_paginated_filing_list` helper walks backward until `--min-filing-date` (default 2009-06-01) or a safety cap is reached. Still no real IC result -- re-ingestion with the fixed script is the account owner's next step. **UPDATED (Session 36 continued further, ADR-0089)**: the account owner re-ran ingestion with the ADR-0088 fix -- 13798 transactions (up from 4877), but the raw-IC screen STILL returned `observations=0`. A live diagnostic proved why: `dateb` does not filter this endpoint's `output=atom` response at all -- a request with `dateb` set to the oldest filing-date already seen came back byte-for-byte identical to the un-paginated first page (100/100 accession numbers overlapping, 0 new). Replaced with `start` (EDGAR's classic 0-based offset), verified for real before being trusted: `start=100` against AAPL's live filing history returned exactly the next 100 filings with 0 overlap against `start=0`. `fetch_form4_filing_list`'s pagination parameter moves from Tier 2 (ADR-0088's untested assumption) to Tier 1 (ADR-0089, actually verified). Still no real IC result -- re-ingestion with this corrected script is the account owner's next step. **UPDATED (Session 36 continued further, ADR-0087)**: the account owner's second parallel direction this session, "apply ML to factor combination -- strengthen regularization + increase data" -- both changed together, before either's own result exists. `MLStrategyParameters.train_window_months` (the `ml_ols`/`ml_ridge` candidates' own in-strategy fit lookback) extended 60 -> 84 months (5 -> 7 fiscal-year snapshots), directly motivated by ADR-0043 Decision 5's own real finding that `ml_ridge` (regularized) outperformed `ml_ols` on fold-consistency (58% vs 53%) on what that ADR itself called "little data." `linear_model.CANDIDATE_RIDGES` (the ridge-CV search grid `ml_ridge` selects from) widened upward from a 100.0 ceiling to include 500.0/1000.0, every weaker prior candidate kept. Deliberately did not add new factor scores to the ML feature set (still the original 6) -- that would need reworking every ML test fixture for the newer factors' extra XBRL concepts (and, for `sue`, an entirely different quarterly shape), a materially larger change kept out of this ADR's scope. **UPDATED (Session 36 continued further)**: real result is in (87-symbol `RESEARCH_UNIVERSE_STAGE4`, without `--insider-db-path` yet) -- `ml_ols` reaches 55% positive folds (DSR=0.6418), `ml_ridge` reaches 53% (DSR=0.5796); neither clears the CANDIDATE bar, same conclusion as before. Recorded honestly rather than smoothed over: this run shows `ml_ols` slightly AHEAD of `ml_ridge`, the opposite ordering from ADR-0043 Decision 5's original finding that motivated widening the regularization search -- though the gap in both directions is small enough (~1-3 folds out of 60) to plausibly be noise, and the universe itself changed between the two runs (STAGE3 63 symbols vs STAGE4 87), a real confound this comparison cannot isolate out. Overall pool conclusion unaffected: PBO=7.14%, `altman_z`/`rank_average_ensemble` remain the only 2 `CANDIDATE`s, both still TEST-negative, zero `VALIDATED` -- see `docs/research/STRATEGY-VALIDATION-REPORT.md`'s own ADR-0087 addendum for the full honest writeup |
