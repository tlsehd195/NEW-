...
**Last Updated:** 2026-09-17 (Session 38 계속 — 보고서 파생 작업 두 번째 항목: Phase 18 `compute_paper_performance_report`를 실제 스케줄 진입점 `run_paper_trading_cycle.py`에 배선(ADR-0136). 배선 과정에서 진짜 숨어있던 갭 발견: `session.adapter.accounting`에 `mark_to_market`를 호출하는 실제 호출자가 지금까지 전혀 없어서 `value_series`/`turnover()`가 항상 비어있었음 — `orchestration.paper_runner.run_cycle`이 사이클마다 이미 계산해두던 reference price를 재사용해 매 사이클 한 번 mark_to_market 호출(ADR-0096과 동일한 "무조건 배선" 처리). 하지만 `PaperTradingSession.restore()`가 fill/order는 재생하지만 mark_to_market 호출 자체는 재생하지 않는다는 걸 확인 → equity_history는 이미 검증된 내구성 있는 출처(risk_repository의 RiskCheckedPosition.risk_state.portfolio_value)로 구성하고, turnover는 누적 분자(내구성 있음)와 이번 프로세스만의 분모(내구성 없음)를 섞으면 재개 실행마다 값이 부정확해질 게 확실해서 조작 대신 정직하게 None("not_supplied")으로 남김(RULE 0.8). `DuckDBPaperPerformanceReportRepository`(이미 존재하던 Phase 18 리포지토리, 실제 호출자는 이번이 처음)로 실제 영속화, `--resume` 재실행마다 새 리포트 누적(덮어쓰지 않음). JSON 리포트와 디스코드 알림(ADR-0135) 양쪽에 Sharpe/Sortino/최대낙폭/총수익률 자동 노출. 새 테스트 9개 추가, 전체 스위트 재통과 후 커밋/푸시/PR/병합 예정. 나머지 항목(ADR 중복번호 정리 → PENDING 주문 TTL → alphalens-reloaded/quantstats 연동 → 기업활동 처리 → T+1 체결 수정 → 소수점 사이징 → features 로깅 배선)은 계속 쉬운 순서대로 진행 예정 — 아래 "### Completed (Session N — ...)" 구조적 기록 참고 — 이어서 ADR 중복번호 정리 항목 착수 전 재확인: main에는 실제 중복 없음(0126-0129 전부 파일 하나씩), 대신 미병합 브랜치 claude/ruflo-features-review-hqrvqa가 자체적으로 다른 내용의 ADR-0032를 쓰고 있어 나중에 그 브랜치를 병합할 때 진짜 번호 충돌이 생긴다는 걸 확인·기록만 함, main 자체는 변경 없음)
