...
**Last Updated:** 2026-09-17 (Session 38 계속 — 쉬운거부터 순서대로의 네 번째 항목: `PaperBrokerAdapter.advance_simulation`/`PaperTradingSession.advance`가 이 저장소 어디에서도 실제로 호출된 적이 없었다는 걸 저장소 전체 검색으로 직접 확인 → `PaperTradingConfig.max_participation` 기본값 10%(그날 봉 거래량의 10%) 제한 때문에 큰 주문은 제출일에 부분체결만 되고 그 뒤로 재시도하는 호출자가 없어서 영원히 PARTIAL_FILLED로 남던 실제 "좀비 주문" 버그를 `orchestration.paper_runner.run_cycle` 맨 앞에서 `session.advance(as_of_time)`을 무조건 호출하게 고쳐서 해결(ADR-0137). 다만 advance()로 발생하는 지연 체결은 아직 Trade Journal에 반영 안 되는 한계(원래 주문의 DecisionSnapshot을 안전하게 재조회할 방법이 없음, 새 영속 인덱스가 필요해서 범위 밖으로 명시적 유보 — reentry-cooldown이 이런 지연 SELL 체결을 실제 청산으로 못 봄)는 정직하게 문서화만 하고 고치지 않음. 새 테스트 1개(직접 10,000주 주문 제출 → 부분체결 2,000주 확인 → 빈 security_ids로 run_cycle 호출해 같은 주문이 4,000주까지 재체결되는지 확인, 새 결정 없이 순수하게 이 수정만 격리해서 검증), 전체 스위트 재통과 후 커밋/푸시/PR/병합 예정. 나머지 항목(alphalens-reloaded/quantstats 연동 → 기업활동 처리 → T+1 체결 수정 → 소수점 사이징 → features 로깅 배선)은 계속 쉬운 순서대로 진행 예정 — 아래 "### Completed (Session N — ...)" 구조적 기록 참고)
