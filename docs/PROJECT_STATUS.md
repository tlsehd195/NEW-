...
**Last Updated:** 2026-09-13 (Session 37 계속 — 코드스페이스 예산 초과로 구글 코랩으로 이전해 87종목 가격/기업활동/재무제표 실측 수집 완료; 내부자거래(Form 4) 수집 중 실제 원인(구식 CGI-bin 목록 엔드포인트만 불안정, 재무제표가 쓰던 최신 data.sec.gov REST API는 0건 실패) 발견해 `_fetch_paginated_filing_list`를 data.sec.gov 기반으로 교체(ADR-0132, 아직 실제 응답으로 검증 전); 오래된 미병합 브랜치 27개 정리(main/조상 확인 후 삭제) + 앞으로 작업 단위마다 main에 병합하는 규칙을 `CLAUDE.md`에 기록; 라인 8 mega-line은 여전히 ADR-0096까지의 역사적 기록이며 이어붙이지 않음 — 최신 세션은 아래 "### Completed (Session N — ...)" 구조적 기록 참고)
