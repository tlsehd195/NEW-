...
**Last Updated:** 2026-09-17 (Session 38 계속 — 쉬운거부터 순서대로의 일곱 번째 항목: 소수점 매수 사이징 --lot-size CLI 플래그 추가(ADR-0140). 코드 작성 전 조사 결과 실제 갭이 예상보다 훨씬 좁았음을 확인: `risk.sizing.DeterministicPositionSizer`는 이미 `PositionSizingConfig.lot_size`(이미 일반 float, 1로 하드코딩 안 됨)로 나눗셈·floor 하는 공식을 쓰고 있어서 lot_size를 1보다 작게 주면 이미 소수점 수량이 나오고, 주문/체결/회계 파이프라인 전체(ValidatedOrder.quantity/Fill.quantity/Position.quantity, 토스 어댑터의 `str(order.quantity)` 직렬화까지)에서 int() 캐스팅을 저장소 전체 검색으로 찾아봐도 전혀 없음을 확인 — 진짜 갭은 `orchestration.paper_strategies.build_run_cycle_components`가 `PositionSizingConfig()`를 내부에서 항상 새로 만들어서 lot_size에 도달할 방법이 아예 없었던 배선 문제뿐이었음. `sizing_config` 옵션 파라미터 추가(기본값은 그대로 1.0, 기존 호출자 전부 영향 없음) + `run_paper_trading_cycle.py`에 `--lot-size` 플래그 신설(체크섬에도 반영). 실제로 0.01 lot_size로 end-to-end 돌려서 진짜 소수점 보유수량이 나오는 것까지 직접 확인. 기업활동 처리(#35)는 이전에 발견한 위험(영속 중복방지 없이 배선하면 재개 실행마다 분할이 반복 적용돼 포지션이 조용히 망가짐) 때문에 계속 보류 중. 새 테스트 5개, 전체 스위트 재통과 후 커밋/푸시/PR/병합 예정. 나머지 항목(T+1 체결 수정 → features 로깅 배선)은 계속 진행 예정 — 아래 "### Completed (Session N — ...)" 구조적 기록 참고)
