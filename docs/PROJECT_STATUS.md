...
**Last Updated:** 2026-09-17 (Session 38 계속 — 쉬운거부터 순서대로의 다섯 번째 항목: quantstats HTML 티어시트 배선(ADR-0138) — 외부 보고서가 추천한 "활발히 관리되는" quantstats-reloaded 포크를 그냥 믿지 않고 실제 설치해서 이 프로젝트의 실제 사용 형태(단일 전략, 벤치마크 없음 — S&P500 데이터 미수집, ADR-0005)로 직접 테스트 → quantstats-reloaded==0.1.0의 reports.html()이 정확히 이 케이스에서 재현 가능한 ValueError로 크래시하는 걸 소스까지 추적해서 확인, 원본 quantstats==0.0.81은 동일 테스트(1~300개 포인트)에서 문제없이 작동함을 확인 → 원본을 채택. `scripts/generate_paper_performance_tearsheet.py` 신설(선택적 [reporting] extra, pandas/numpy는 src/에 여전히 전혀 안 들어감), ADR-0136이 이미 만든 동일한 실제 내구성 있는 equity curve(risk_repository 기반)를 그대로 재사용. 새 테스트 7개, 전체 스위트 재통과 후 커밋/푸시/PR/병합 예정. 아직 GitHub Actions 스케줄에는 연결 안 함(수동 스크립트로만 존재) — 다음 자연스러운 단계로 남김. 나머지 항목(alphalens-reloaded 연동 → 기업활동 처리 → T+1 체결 수정 → 소수점 사이징 → features 로깅 배선)은 계속 쉬운 순서대로 진행 예정 — 아래 "### Completed (Session N — ...)" 구조적 기록 참고)
