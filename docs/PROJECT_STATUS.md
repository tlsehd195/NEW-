...
**Last Updated:** 2026-09-17 (Session 38 계속 — 쉬운거부터 순서대로의 여덟 번째 항목: `DecisionSnapshot.features` 실제 배선(ADR-0141) — 사용자가 인용했던 외부 평가 보고서의 정확한 문구("MeanRewardBaselineTrainer/LinearRegressionTrainer가 어떤 전략도 features를 로깅하지 않아서 실훈련 샘플 0건")를 실제로 고침. 전체 체인을 끝까지 직접 추적한 결과: `DecisionSnapshot.features` 필드부터 `storage.trade_journal_repository`의 직렬화, `trade_journal.experience.build_experience_records`의 features 복사, `learning.labeling.Labeler`의 LabeledSample.features 연결, `run_learning_cycle.py`의 `--feature-id` 플래그까지 읽기 쪽 전체가 이미 정확하게 완성돼서 대기 중이었고, 진짜 갭은 `orchestration.paper_runner.run_cycle`의 `record_decision(...)` 호출 딱 한 곳이 `features=`를 한 번도 넘긴 적이 없었다는 것뿐이었음. 새 헬퍼 `_prediction_features()`로 PredictionOutput의 이미 계산된 5개 실제 값(expected_return/probability/expected_volatility/uncertainty/confidence)을 넘기고, None인 값은 아예 키를 생략(LinearRegressionTrainer가 키 존재 여부만 체크해서 "key: None"을 넣으면 나중에 산술 연산에서 크래시하는 걸 미리 확인해서 방지). 실제 체결까지 이어지는 e2e 테스트로 진짜 DecisionSnapshot.features가 채워지는 것까지 직접 확인(기존 저장소에 있는 어떤 fixture도 실제 학습 완료(fitted=True)까지 가기엔 청산 거래 수가 부족하다는 것도 정직하게 남김 — 이건 이번 수정의 한계가 아니라 fixture 크기의 한계). 새 테스트 3개 + 기존 테스트 1개 보강, 전체 스위트 재통과 후 커밋/푸시/PR/병합 예정. 남은 항목: 기업활동 처리(#35, 영속 인덱스 필요해서 보류 중)와 T+1 체결 수정(#36) — 아래 "### Completed (Session N — ...)" 구조적 기록 참고)
