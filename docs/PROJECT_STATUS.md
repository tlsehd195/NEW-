...
**Last Updated:** 2026-09-17 (Session 38 계속 — 쉬운거부터 순서대로의 여섯 번째 항목: alphalens-reloaded로 Signal IC 교차검증 스크립트 신설(ADR-0139) — quantstats-reloaded에서 실제 크래시를 발견했던 것과 마찬가지로 이번에도 그냥 신뢰하지 않고 직접 검증: alphalens-reloaded 자체는 크래시하지 않지만, 이 프로젝트의 기존 월단위 리밸런스 캐덴스(--step-months)로 factor를 넘기면 alphalens가 factor 날짜와 가격 인덱스의 합집합에서 단일 거래 캘린더를 추론하려다가 "Inferred frequency ... does not conform" ValueError로 실패한다는 걸 직접 재현·확인(개별 날짜를 실제 거래일로 스냅해도 안 고쳐짐 — 밀도 자체의 근본적 불일치). 해결: alphalens 쪼에만 [--start,--end) 구간의 모든 실제 거래일마다 factor를 매일 평가하도록 만들고, 이 프로젝트 자신의 compute_ic_series는 기존 --step-months 그대로 유지 — 두 수치가 서로 다른 리밸런스 밀도를 잰다는 걸 출력 문구에도 명시해서 숨기지 않음. 새 선택적 extra `research`(alphalens-reloaded만) 추가, src/는 여전히 pandas/numpy 없음. 가격전용 팩터 16개만 배선(모멘텀 전략 클래스 기반은 추후로 유보). 새 테스트 6개, 전체 스위트 재통과 후 커밋/푸시/PR/병합 예정. 나머지 항목(기업활동 처리 → T+1 체결 수정 → 소수점 사이징 → features 로깅 배선)은 계속 쉬운 순서대로 진행 예정 — 아래 "### Completed (Session N — ...)" 구조적 기록 참고)
