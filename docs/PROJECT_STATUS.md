...
**Last Updated:** 2026-09-17 (Session 38 계속 — "쉬운거부터 순서대로" 8개 항목 완료 후, 사용자 본인이 직접 로컬/계정에서 실행해야 하는 나머지 항목들을 코드 변경 없이 기록만 함(#39). 이번 세션에서 완료: (1) 디스코드 웹훅 알림(ADR-0135), (2) 성과 리포트 배선+mark_to_market 갭 수정(ADR-0136), (3) ADR 중복번호 재확인(실제 중복 없음), (4) advance_simulation 배선으로 좀비 주문 재시도 버그 수정(ADR-0137), (5) quantstats 티어시트(quantstats-reloaded 실제 크래시 버그 발견, ADR-0138), (6) alphalens 교차검증(캐덴스 불일치 발견·해결, ADR-0139), (7) 소수점 매수 사이징 --lot-size(ADR-0140), (8) DecisionSnapshot.features 배선(ADR-0141). 남은 두 항목은 진짜로 더 어려워서 보류 중: 기업활동 처리(#35, 영속 중복방지 인덱스 필요)와 동일봉체결→T+1 체결 수정(#36, PENDING 주문 상태·advance_simulation·평균단가 전반에 영향을 주는 더 큰 아키텍처 변경이라 착수 전 사용자 확인이 필요하다고 판단, 이번 세션에서는 시작 안 함). 아래 "### Completed (Session N — ...)" 구조적 기록 참고)
...
1. **디스코드 웹훅 알림 활성화** (ADR-0135) — Discord 채널 설정 → 통합 → 웹후크에서 실제 웹훅 생성 → 그 URL을 GitHub 저장소 Settings → Secrets and variables → Actions에 `DISCORD_WEBHOOK_URL` 이름으로 등록. 등록 전까지는 워크플로 스텝이 조용히 스킵되기만 하고 아무것도 깨지지 않음.
...
### Completed (Session 38 — 12개 외부 보고서 파악 + 브레인스토밍 전체 답변 + 디스코드 웹훅 알림 구현, ADR-0135)
