...
**Last Updated:** 2026-09-12 (Session 37 계속 — SEC 13F 팩터(institutional_ownership_change_score) 최초 실측 검증 완료: 직전 분기 데이터까지 확보해 같은 DB에 저장 후 실제 함수 직접 호출, ALLY/BAC/KO/AAPL 4종목 전부 `None`이 아닌 실측 값 확인(부호도 공개된 실제 사실과 일치) — 이 과정에서 윈도우 `python3`가 Microsoft Store 스텁으로 잘못 연결되어 조용히 아무것도 안 하던 환경 문제 발견·수정(`python` 사용으로 전환); 라인 8 mega-line은 여전히 ADR-0096까지의 역사적 기록이며 이어붙이지 않음 — 최신 세션은 아래 "### Completed (Session N — ...)" 구조적 기록 참고)
